...
Every document in this repository, and which question it answers. Start with
the row that matches why you are here.
...
|---|---|
| **Reviewing this in twenty minutes** | [../README.md](../README.md) claim table → [DIAGRAMS.md](DIAGRAMS.md) → [LIMITATIONS.md](LIMITATIONS.md) → [FAQ.md](FAQ.md) |
...
| **Trying to run it** | [SETUP.md](SETUP.md) → [RUNBOOK.md](RUNBOOK.md) → [TROUBLESHOOTING.md](TROUBLESHOOTING.md) |
...
| **Looking for the flaw** | [LIMITATIONS.md](LIMITATIONS.md) → [../DECISIONS.md](../DECISIONS.md) → [CASES.md](CASES.md) → [TESTING.md](TESTING.md) |
...
### Orientation — hand-written, no measured numbers
...
| [JOURNEY.md](JOURNEY.md) | What did this project actually do, phase by phase, and what did each phase change? |

### Contracts — the design, binding on contributors
...
| [TASKS.md](TASKS.md) | The phased build order and the gate at the end of each phase |
...
| Document | Answers |
|---|---|
| [../DECISIONS.md](../DECISIONS.md) | 118 append-only entries. "Why did you do it that way?" — including the reversals |
| [METHODS.md](METHODS.md) | Estimators, bootstraps, null bands, and where each came from |
| [LIMITATIONS.md](LIMITATIONS.md) | Scope, declared gaps, open items. **Read before quoting anything** |
...
| Document | Answers |
|---|---|
| [DEMO.md](DEMO.md) | The two-pane demo this build exists to produce |
| [DEMO_SCRIPT.md](DEMO_SCRIPT.md) | The beat-by-beat script and what is on screen |
...
1. **Every number is computed by code.** If it cannot be traced to an artifact
   in `results/`, it does not go in a document. The README's claim table names
   the artifact *and the field* for each of its numbers, and `make verify`
   resolves all of them. A number edited by hand fails the build.
2. **The orientation pages above carry no measured numbers at all.** They point
   at the generated documents instead, so a re-run cannot leave them stale.
   Structure, procedure and vocabulary change with the code, not with the run.
3. **`DECISIONS.md` is append-only.** A reversed decision is superseded by a new
   entry that links back — never edited away. That is why entries 108/109 and
   116 read as corrections rather than as history that never happened.
4. **Prose documentation is written at the end, not during the build**
   (`CLAUDE.md`). `DECISIONS.md` is the sole exception, because it is the one
   document that cannot be reconstructed later.
5. **A figure about the world needs a register entry.** Court awards, fines,
   standard clause numbers — [EXTERNAL_FIGURES.md](EXTERNAL_FIGURES.md), and a
   test enforces it against the proposal.
