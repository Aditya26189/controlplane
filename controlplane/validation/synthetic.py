...
:mod:`controlplane.detectors.aggregation` code, so what pooling does to that signal as
...
def synthetic_evalset(
    *,
    ...
    return EvalSet(
        ...
        construction={
            # FROZEN STRING -- deliberately still says "src". `construction`
            # feeds `EvalSet.content_hash`, the content hash IS the envelope
            # id, and the envelope id is the third element of the warrant key
            # (invariant 1). Renaming the package here would silently re-issue
            # every synthetic fixture under a new envelope and orphan the
            # warrants in results/fixtures/. This literal is a frozen identity,
            # not a live import path. See DECISIONS.md 095.
...
def synthetic_cache(
    ...
    *incorrect*. The bump is pooled by the actual
    :func:`~controlplane.detectors.aggregation.aggregate` functions, so the difference
    ...
    return ExtractionCache(
        ...
        extra={
            ...
            # id, and the envelope id is the third element of the warrant key
            # (invariant 1). Renaming the package here would silently re-issue
            # every synthetic cache under a new envelope and orphan the
            # warrants in results/fixtures/. This literal is a frozen identity,
            # not a live import path. See DECISIONS.md 095.
