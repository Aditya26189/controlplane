...
**Contracts** (`AGENTS.md`, `SPEC.md`, `TASKS.md`) — a doc change ships **in the same commit** as the code change that made it necessary. Never a trailing "update docs" commit. If code contradicts `SPEC.md`, stop: either the code is wrong or the spec needs updating deliberately, first, with the reasoning logged.
