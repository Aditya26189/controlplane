...
Clone rather than upload, so the notebook is running the same code that is
committed and the artifacts record a commit that exists.
"""
        ),
        code(
            """
# If you have pushed the repo, clone it. Otherwise upload it as a Kaggle Dataset
# and point PROJECT at the mounted path.
import os, sys
from pathlib import Path

PROJECT = Path("/kaggle/working/controlplane")
if not PROJECT.exists():
    # Replace with your repository URL, or attach the repo as a Dataset.
    !git clone --depth 1 https://github.com/YOUR_USER/YOUR_REPO.git /kaggle/working/controlplane
PROJECT = PROJECT / "round 2" if (PROJECT / "round 2").exists() else PROJECT
