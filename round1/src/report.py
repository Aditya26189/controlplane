...
# Handover document
#
# Complements RESULTS.md rather than duplicating it. RESULTS.md is the formal
# record in SPEC.md §13's order; this is orientation for someone who was not
# here and may have to present the work -- what it means, what bounds it, what
# to say and what not to say. It links to RESULTS.md for the full tables.
# --------------------------------------------------------------------------- #


def render_handover_md(artifacts: dict[str, Any], config: Config) -> str:
    """Render ``docs/HANDOVER.md`` for a teammate joining cold.

    Every number is read from ``results/``; nothing is typed. The framing
    sections exist because a handover usually fails at the retelling rather
    than the reading -- the lift is easy to quote without its ceiling, and the
    probe is easy to describe as working on models it cannot reach.
    """
    extract = artifacts["extract_meta"]
    probe_test = artifacts["probe_test"]
    economics = artifacts["economics"]["economics"]
    ...
    log = artifacts.get("test_scoring_log")
    logged = log.get("n_scorings", 0) if log else 0

    lines: list[str] = []
    add = lines.append

    add("# Handover — cascade economics")
    add("")
    add(
        "*Generated by `scripts/06_handover.py` from `results/`. Every number "
        "here is read from an artifact; none is typed by hand.*"
    )
    add("")
    add(
        "**What this is.** We measured whether a linear probe on a model's "
        "internal state, read *before it generates a single token*, can pick "
        "which responses are worth spending an expensive checker on. It can. "
        "This page is what that means, what bounds it, and how to talk about it."
    )
    add("")
    add(
        f"`{model['name']}` · {model['quantization'].upper()} · "
        f"{provenance['device']['device_name']} · seed {provenance['seed']} · "
        f"config `{provenance['config_hash']}` · commit "
        f"`{str(provenance['git_commit'])[:7]}`"
    )
    add("")
    add("---")
    add("")
    ...
    add("| | Value | 95% CI |")
    add("|---|---|---|")
    add(f"| Test AUROC | **{fmt(test['auroc'], 4)}** | {fmt_ci(boot['auroc'], 4)} |")
    add(f"| Lift (`R/f`) | **{fmt(economics['lift'], 2)}×** | {fmt_ci(boot['lift'], 2)} |")
    if ceiling:
        add(
            f"| Ceiling on this data | {fmt(ceiling['max_attainable_lift'], 2)}× "
            f"| we reached {fmt_pct(ceiling['fraction_of_ceiling_achieved'], 1)} |"
        )
    add(f"| Probe cost | {fmt(comparison['probe_median_us'], 0)} µs "
        f"| vs {fmt(comparison['generation_median_ms'] / 1000, 1)} s to generate |")
    add("")
    if ceiling:
        add(
            "**Read the lift with its ceiling — this is the one thing not to "
            "misquote.** `lift = R/f = precision / base_rate`, and precision "
            "cannot exceed 1, so lift is capped at `1/base_rate`. The model is "
            f"wrong on {fmt_pct(ceiling['measured_base_rate'], 1)} of this "
            f"benchmark, capping lift at "
            f"{fmt(ceiling['max_attainable_lift'], 2)}× for *any* probe. We "
            f"reached {fmt_pct(ceiling['fraction_of_ceiling_achieved'], 1)} of "
            "what was available."
        )
        add("")
        add(
            "The transferable quantity is therefore the **AUROC**, not the lift. "
            "The lift is what that ranking quality is worth on a workload where "
            "the model fails this often."
        )
        add("")
        add("> **How to quote it**")
        add("> ")
        add(
            f"> \"{fmt(economics['lift'], 2)}× more errors caught at the same "
            "judge budget — which is "
            f"{fmt_pct(ceiling['fraction_of_ceiling_achieved'], 0)} of the "
            "maximum this benchmark allows.\""
        )
        add("> ")
        add(
            "> The second clause is not a hedge. Without it the number looks "
            "weak; with it, it reads as near-optimal on a hard test."
        )
        add("")

    # -- 2. mechanism ------------------------------------------------------- #
    add("## 2. What it actually does")
    add("")
    add(
        f"A model reads a question and has generated nothing yet. At layer "
        f"{probe['layer']} of {model['num_hidden_layers']}, we take the "
        f"residual-stream vector at the final prompt token — "
        f"{model['hidden_size']:,} numbers that fall out of the prefill pass the "
        "model performs anyway. A logistic regression scores it. Responses above "
        "a threshold go to the expensive checker; the rest don't."
    )
    add("")
    add(
        "The probe is a **trigger, not a verdict**. It never blocks anything, "
        "which is why it is tuned for recall and why low precision would be "
        "acceptable: a false positive wastes one judge call, a false negative "
        "lets a user act on a wrong answer."
    )
    add("")
    add(
        f"It adds **no extra forward pass**. The quoted "
        f"{fmt(comparison['probe_median_us'], 0)} µs is the full scikit-learn "
        f"call; the arithmetic alone is "
        f"{fmt(comparison.get('raw_dot_product_median_us'), 1)} µs. We quote the "
        "slower figure deliberately — it is the conservative one."
    )
    add("")
    add(policy_table_markdown(economics))
    add("")
    add(
        "All three policies spend the same judge budget. Coverage and verdict "
        "are different things, and that gap is the whole result."
    )
    add("")

    # -- 3. why believe it -------------------------------------------------- #
    add("## 3. Why believe it")
    add("")
    add(
        "The failure that would have quietly ruined this experiment is padding. "
        "With right padding, position −1 of a batch is a pad token, every "
        "activation is meaningless, nothing raises an error, and you get an "
        "AUROC near 0.5 that reads as \"the idea doesn't work\"."
    )
    add("")
    add(
        "So every run measures the same batch twice — once correctly, once "
        "**deliberately broken** — and requires the broken one to fail:"
    )
    add("")
    add("| Padding | Relative L2 | Cosine | Verdict |")
    add("|---|---|---|---|")
    add(
        f"| Left (correct) | {equivalence['max_relative_l2']:.3e} "
        f"| {fmt(equivalence['min_cosine_observed'], 6)} | accepted |"
    )
    if control:
        add(
            f"| Right (deliberate fault) | {control['max_relative_l2']:.3f} "
            f"| {fmt(control['min_cosine'], 4)} | **rejected** |"
        )
    add("")
    add(
        f"Limits are relative L2 ≤ {equivalence['relative_tolerance']} and cosine "
        f"≥ {equivalence['min_cosine']}, sitting far from both regimes. This "
        "turns the check from \"it passed\" into \"it still rejects the fault "
        "it exists for, on this hardware\"."
    )
    add("")
    add(
        f"**Selection discipline.** Layer, regularisation strength and threshold "
        f"were all chosen on "
        f"validation; test was never consulted for any of them. It has been "
        f"scored {logged + 1} times in total — once before the audit log "
        "existed, and every scoring is published, with the one re-scoring "
        "pre-registered in `DECISIONS.md` 016 before it ran."
    )
    add("")
    add(
        f"Splits are by question, after removing {data['duplicates_dropped']:,} "
        f"duplicate questions from {data['rows_loaded']:,} rows. That dedup "
        "matters: roughly 80% of questions in this dataset appear twice, so an "
        "example-level split would have put the same question in train and test."
    )
    add("")

    # -- 4. numbers --------------------------------------------------------- #
    add("## 4. The measurements")
    add("")
    add(
        f"Selected **layer {probe['layer']}, C = {probe['C']:g}** on validation "
        f"AUROC {fmt(probe['val_auroc'], 4)}. The winner sits *inside* the grid "
        f"(`winner_at_grid_boundary` = {sweep.get('winner_at_grid_boundary')}); "
        "an earlier run picked the smallest value at every layer, meaning the "
        "search had hit a wall rather than found an optimum (`DECISIONS.md` 016, 017)."
    )
    add("")
    add(f"![Layer sweep](../{config.paths.results_dir}/layer_sweep.png)")
    add("")
    add(f"![Test ROC](../{config.paths.results_dir}/roc_curve.png)")
    add("")
    add(
        f"At the frozen threshold on {test['n']} held-out questions: "
        f"{test['tp']} true positives, {test['fp']} false positives, "
        f"{test['fn']} missed, {test['tn']} correctly passed. Base error rate "
        f"{fmt(test['base_rate'], 4)}."
    )
    add("")
    add(
        f"Extraction ran {extraction['n_examples']:,} examples in "
        f"{fmt_duration(extraction['total_seconds'])} at "
        f"{fmt(extraction['examples_per_second'], 2)} examples/s."
    )
    add("")
    add(
        f"Full tables, bootstrap intervals and the base-rate projection are in "
        f"[`{config.paths.results_dir}/RESULTS.md`](../{config.paths.results_dir}/RESULTS.md)."
    )
    add("")
    ...
    add("")
    add("> **The sharpest question you will be asked**")
    add("> ")
    add(
        "> \"Enterprises use models through an API. You can't read activations "
        "through an API.\" Correct, and it needs answering head-on: the probe "
        "tier requires weight access. The *cascade* generalises — where you "
        "don't own the weights, the cheap tier degrades to output-layer "
        "signals. Don't claim universality."
    )
    add("")
    for item in limitations(artifacts, config):
        add(f"- {item}")
    ...
    # -- 6. framing --------------------------------------------------------- #
    add("## 6. If you're presenting this")
    add("")
    add("**Say**")
    add("")
    add(
        f"- \"The probe ranks wrong answers at AUROC {fmt(test['auroc'], 3)}. At a "
        f"{fmt_pct(test['flag_rate'], 0)} judge budget that's "
        f"{fmt(economics['lift'], 1)}× more errors than random sampling"
        + (
            f" — {fmt_pct(ceiling['fraction_of_ceiling_achieved'], 0)} of the "
            "ceiling this benchmark allows.\""
            if ceiling
            else ".\""
        )
    )
    add(
        f"- \"It costs {fmt(comparison['probe_median_us'], 0)} microseconds "
        f"against a {fmt(comparison['generation_median_ms'] / 1000, 1)} second "
        "generation, and adds no forward pass.\""
    )
    add("- \"We deliberately broke the padding to prove the check still catches it.\"")
    add("")
    add("**Don't say**")
    add("")
    add(
        f"- \"{fmt(economics['lift'], 1)}× better than random\" — without the "
        "ceiling it sounds weak rather than near-optimal."
    )
    add(
        "- \"It works on any model\" — it needs weight access. Say so before "
        "someone else does."
    )
    add(
        "- \"This catches hallucinations\" — it is a correlational classifier "
        "over activations, not a truth detector."
    )
    add("")

    # -- 7. next ------------------------------------------------------------ #
    add("## 7. What's still open")
    add("")
    add(
        "- **The API-access design note.** The cascade with the probe as one "
        "tier, and what the cheap tier becomes without weights. Fastest of the "
        "three, and it decides whether the rest is taken seriously."
    )
    add(
        "- **Policy layer and tiered decisions.** One measured ROC, different "
        "operating points per use case — a customer-facing bot and an internal "
        "copilot want different budgets on the same curve. Needs no new science."
    )
    add(
        "- **GSM8K negative control.** Published work says this method falters "
        "on mathematical reasoning; reproducing that deliberately is more "
        "credible than citing it."
    )
    add("")
    add("## Reproducing")
    add("")
    ...
    add("python -m pytest tests/ -q                      # no GPU, no downloads")
    add("python scripts/run_all.py --from 02             # ~1 min from saved activations")
    add("python scripts/run_all.py --config config.yaml  # full run, needs a GPU")
    add("```")
    add("")
    add(
        "Everything except extraction runs on a laptop. The full run needs a "
        f"free Kaggle T4 for about "
        f"{extraction['total_seconds'] / 3600:.1f} hours; "
        "`notebooks/run_on_kaggle.ipynb` walks through it."
    )
    add("")
    add(
        "Methodology and every contested choice are logged in "
        "[`DECISIONS.md`](../DECISIONS.md); the invariants are in "
        "[`CLAUDE.md`](../CLAUDE.md)."
    )
    return "\n".join(lines) + "\n"
